...
class PreloadVideoBackground extends StatefulWidget {
  ...
  const PreloadVideoBackground({
    ...
    this.preloadTimeout = const Duration(seconds: 5),
    this.enablePreload = true,
  });

  @override
  State<PreloadVideoBackground> createState() => _PreloadVideoBackgroundState();
}

class _PreloadVideoBackgroundState extends State<PreloadVideoBackground>
  ...
  Future<void> _preloadVideo() async {
    try {
      ...
      await _controller!.initialize().timeout(
        widget.preloadTimeout,
        onTimeout: () {
          throw Exception('视频预加载超时');
        },
      );
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        ...
        else if (_isVideoReady && _controller?.value.isInitialized == true)
          FadeTransition(
